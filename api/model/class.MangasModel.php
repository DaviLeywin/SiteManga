...
class Mangas {
    ...
    function AlterarTitulo(string $Titulo){
        if(!BaseValidator::ValidarTamanho($Titulo, 200)){
            throw new InvalidArgumentException("TITULO exedeu o tamanho maximo de 200 caracteres!");
        }
        $this->TITULO = $Titulo;
    }
    ...
    function AlterarStatus(string $Status){
        if(!BaseValidator::ValidarTamanho($Status, 50)){
            throw new InvalidArgumentException("STATUS exedeu o tamanho maximo de 50 caracteres!");
        }
        $this->STATUS = $Status;
    }
